...
\begin{tikzpicture}
  ...
  % Decision boundary line with annotation
  \draw [ultra thick] (-1.5,-3) -- (5,1.3333)
    node [above right] {\large$\theta^T x + \theta_0 = 0$};

  % Arrow representing the parameter vector
  \draw [thick, teal, -latex] (2,-0.6667) -- (1,0.8883)
    node [black, below left] {$\theta$};

  % Labeled points (using standard math mode syntax)
  \node [above right, xshift=.3em] at (3,2) {\large$\{x\}^{(1)}$};
  \node [above right, xshift=.3em] at (4,-1) {\large$\{x\}^{(2)}$};

  % Draw plus markers
  \foreach \Point in {(3,2), (1.8,3.5), (-1,3), (.8,-.6), (-1.4,-.9), (1.5, 1.5)}{
    \pic at \Point {plus};
  }
...
$$
J(\theta, \theta_0) = \frac{1}{n} \sum_{i=1}^n \mathcal{L}_{01}(\text{sign}(\theta^Tx^{(i)} + \theta_0),
  y^{(i)})\;\;.
$$ 
