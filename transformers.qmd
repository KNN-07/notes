...
:::{.column-margin}
You might hear people refer to ‘softmaxed attention scores’ as ‘attention weights.’ We intentionally avoid this terminology because these values are not network weights—they’re not directly learned parameters but rather computed from learned quantities.
:::
...
This is a weighted sum of the value vectors using the softmax'd attention scores. The attention output $z_i$ is a vector that captures the information from all tokens in the sequence, weighted by how much attention the $i$th token should pay to each of them.
