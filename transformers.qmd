...
where $V\in \mathbb{R}^{n \times d_k}$ is the matrix of value vectors stacked row-wise, and $Z \in \mathbb{R}^{n \times d_k}$ is the output, whose $i$th row corresponds to the attention output for the $i$th query (i.e., $z_i$). More compactly, we often see a combined notation incorporating attention score, Softmax, and final weighted sum:
...
$$
where the concatenation operation will concataneing all $Z^h$ horizontally, yielding a matrix of size $n\times Hd_k$, and $W^O \in \mathbb{R}^{d_k \times d_o}$ is a final output linear projection matrix.
