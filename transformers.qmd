...
In the above section, we see how self-attention behaves for a single query. Since self-attention is independent for every query $q_i$ associated with the $i$th input $x_i$, it means we actually can calculate all outputs $z_i$ at the same time, using a matrix form. For clearness, we first introduce the $Q\in\mathbb{R}^{n\times d_k}$ query matrix, $K\in\mathbb{R}^{n\times d_k}$ key matrix, and $V\in\mathbb{R}^{n\times d_k}$ value matrix as:
...
\end{aligned}
$$It should be straightforward to understand, the $Q$, $K$, $V$ matrices simply stack $q_i$, $k_i$, and $v_i$ in a row-wise manner, respectively. Now, the the full attention matrix $A \in \mathbb{R}^{n \times n}$ is:
...
A = \begin{bmatrix}
  ...
  \end{bmatrix}
\end{aligned}
$${#eq-self_softmax} which often time is written as:
$$\begin{aligned}
A = \text{softmax}\left(
\frac{1}{\sqrt{d_k}}
\begin{bmatrix}
...
\end{bmatrix}
\right)\end{aligned}=\text{softmax}\left(\frac{Q}{K^\top}{\sqrt{d_k}}\right)
$$
