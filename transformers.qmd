...
There are two main strategies for positional embeddings: (i) Learned positional embeddings, where a trainable vector is assigned to each position index (e.g., position 0, 1, 2, ..., N). These vectors are learned alongside all other model parameters and allow the model to discover how best to encode position for a given task. (ii) fixed positional embeddings, such as sinusoidal positional embedding proposed in the original Transformer paper.


Importantly, these positional encodings are added to the token embeddings:

$x_i^\star = x_i + p_i$​
where $x_i$ is the $i$th input token, and $p_i$ is its positional embedding. The $x_i^\star$ will now be fed into the attention layer, so that the input to the attention mechanism now carries information about both what the token is and where it appears in the sequence.
