...
The animation above illustrates the auto-regressive nature of transformers. Below is another example, showing how each token's prediction is conditioned on previously generated tokens, forming a step-by-step probability distribution over the vocabulary.
...
There are additional essential components and enhancements—such as masked attention mechanisms and positional encoding—that further empower transformers. We'll explore these \"bells and whistles\" in greater depth in subsequent discussions.
...
Now, our attention mechanism (attention score) abstractly looks like:

$$
a_{i,j} = q_i^T k_j
$$

That is, our rich representation relies on the dot-product similarity of query-key pairs for any tokens $i$ and $j$.
...
In the above section, we see how self-attention behaves for a single query. Self-attention is actually independent for the $i$th query $q_i$ (which is ssociated with the $i$th input $x_i$), it implies that we  can calculate all outputs $z_i$ ($i=1,2,\ldots,n$) at the same time using a matrix form. For clearness, we first introduce the $Q\in\mathbb{R}^{n\times d_k}$ query matrix, $K\in\mathbb{R}^{n\times d_k}$ key matrix, and $V\in\mathbb{R}^{n\times d_k}$ value matrix:
...
A = \begin{bmatrix}
  ...
  \end{bmatrix}
\end{aligned}
$${#eq-self_softmax} which often time is shorten as:
$$\begin{aligned}
A = \text{softmax}\left(
\frac{1}{\sqrt{d_k}}
\begin{bmatrix}
...
Z=A V
$$
where $V\in \mathbb{R}^{n \times d_k}$ is the matrix of value vectors stacked row-wise, and $Z \in \mathbb{R}^{n \times d_k}$ is the output, whose $i$th row corresponds to the attention output for the $i$th query (i.e., $z_i$). More compactly, we often see a combined notation incoporating attention score, Softmax, and final weighted sum:
...
Now, we introduce the formal math notations. Let us denote the numebr of head as $H$. For the $h$th head, the input $X \in \mathbb{R}^{n \times d}$ is linearly projected into query, key, and value matrices using the projection matrices $W_q^{h}\in\mathbb{d\times d_q}$, $W_k^{h}\in\mathbb{d\times d_k}$, and $W_v^{h}\in\mathbb{d\times d_v}$ (recall that usually $d_q=d_k=d_v):
$$\begin{align*}
Q^{h} &= X W_q^{h} \\
K^{h} &= X W_k^{h} \\
V_{h} &= X W_v^{h}
...
The output of the $i$-th head is $Z^{h}$: ${Z}^h = \text{Attention}(Q^{h}, K^{h}, V^{h})\in\mathbb{n\times d_k}$. After computing all $h$ heads, we concatenate their outputs and apply a final linear projection:
$$
\text{MultiHead}(X) = \text{Concat}(Z^1, \dots, Z^H) W^O
$$
where the concatation operation will concataneing all $Z^h$ horizontally, yielding a matrix of size $n\times Hd_k$, and $W^O \in \mathbb{R}^{d_k \times d_o}$ is a final output linear projection matrix.

### Positional Embeddings 

An extremely observant reader might have been suspicious of a small but very important detail that we have not yet discussed: our introduced attention operation thus far has not encoded the order of the input tokens. For instance, consider the example we provide at the beginning of this chapter, "A robot must obey the orders g"

As we discussed the processing of input data, we framed it as the model will ingest each token, one at a time, and for each one, the model will try to predict the next token in the sequence. Truly, that’s how the model can be designed and used. However, for any given input sequence of text, when the Attention mechanism is deciding how much focus to place on every token, and in turn produce rich representations for each token, the model actually has no clue the current set of tokens are ordered! I deliberately used the word “set” because the tokens are effectively serving as a set (unordered list of items)!

For example, let’s imagine that the following two input sequences were input into a Transformer:

Scenario 1: The brown dog ran away

Scenario 2: The away ran dog brown

In both of these scenarios, when processing any given token, let’s say “brown”, the Attention mechanism would produce identical results (attention scores and subsequent token representations z_i), despite the input words being in different order! Clearly this is sub-optimal, as it’s neglecting to use crucial information. For example, instead of the extreme scenario provided above, imagine if we had:
