...
We seek to measure the goodness of a policy. We do so by defining for a given horizon $h$ and MDP policy $\pi_h$, the "horizon $h$ *value*" of a state, $\mathrm{V}_{\pi}^h(s)$. We do this by induction on the horizon, which is the *number of steps left to go*.
...
$$
\mathrm{V}_{\pi}^1(s) = \mathrm{R}(s, \pi_1(s)) + 0
$${#eq-finite_val_1}

$$
\mathrm{V}_{\pi}^2(s) = \mathrm{R}(s, \pi_2(s)) +  \gamma \sum_{s'}\mathrm{T}(s, \pi_2(s), s')  \mathrm{V}_{\pi}^1(s')
$${#eq-finite_val_2}
...
$$
\mathrm{V}_{\pi}^h(s) = \mathrm{R}(s, \pi_h(s)) + \gamma \sum_{s'}\mathrm{T}(s, \pi_h(s), s')  \mathrm{V}_{\pi}^{h - 1}(s')
$${#eq-finite_value}
...
Then we can say that a policy $\pi$ is better than policy $\bar{\pi}$ for horizon $h$ if and only if for all $s \in \mathcal S$, $\mathrm{V}^{\pi}(s) \geq \mathrm{V}^{\bar{\pi}}(s)$ and there exists at least one $s \in \mathcal S$ such that $\mathrm{V}^{\pi}(s) > \mathrm{V}^{\bar{\pi}}(s)$.
...
An important theorem about MDPs is: in the infinite-horizon case, there exists a stationary optimal policy $\pi^*$ (there may be more than one) such that for all $s \in \mathcal S$ and all other policies $\pi$, we have $$\mathrm{V}^{\pi^*}(s) \ge \mathrm{V}^{\pi}(s) \;\;.$$
