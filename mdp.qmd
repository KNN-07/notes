...
$$
\mathrm{V}^2_{\pi}(s) = R(s, \pi_2(s)) +  \gamma \sum_{s'}T(s, \pi_2(s), s')  \mathrm{V}^1_{\pi}(s')
$${#eq-finite_val_2}
...
$$
\mathrm{V}^h_{\pi}(s) = R(s, \pi_h(s)) + \gamma \sum_{s'}T(s, \pi_h(s), s')  \mathrm{V}^{h - 1}_{\pi}(s')
$${#eq-finite_value}
...
$$\begin{aligned}
  ...
  \nonumber
  \\
             & = R(s, \pi(s)) + \gamma\sum_{s'}T(s, \pi(s), s')\mathrm{V}_{\pi}(s') \; .
\end{aligned}
$${#eq-inf_horiz_value}
...
$$\begin{aligned}
  ...
  \mathrm{Q}^2(s, a) & = R(s, a) + \gamma \sum_{s'}T(s, a, s') \max_{a'} \mathrm{Q}^1(s', a')       \\
  \vdots
  \nonumber                                                                       \\
  \mathrm{Q}^h(s, a) & = R(s, a) + \gamma \sum_{s'}T(s, a, s') \max_{a'} \mathrm{Q}^{h - 1}(s', a')
\end{aligned}
$$ 
...
$$\mathrm{Q}(s, a) = R(s, a) + \gamma\sum_{s'}T(s, a, s')\max_{a'}\mathrm{Q}(s', a') \;\;.$$
...
\WHILE{not converged}
    \FOR{each $s \in \mathcal{S}$ and $a \in \mathcal{A}$}
        \STATE $\mathrm{Q}_{\text{new}}(s,a) \gets R(s,a) + \gamma \displaystyle\sum_{s'} T(s,a,s')\, \max_{a'} \mathrm{Q}_{\text{old}}(s',a')$
    \ENDFOR
