...
Formally, a Markov decision process is $\langle \mathcal S, \mathcal A, T, \mathrm{R},
...
We seek to measure the goodness of a policy. We do so by defining for a given horizon $h$ and MDP policy $\pi_h$, the "horizon $h$ *value*" of a state, $\mathrm{V}^h_{\pi}(s)$. We do this by induction on the horizon, which is the *number of steps left to go*.
...
$$
\mathrm{V}^1_{\pi}(s) = \mathrm{R}(s, \pi_1(s)) + 0
$${#eq-finite_val_1}

$$
\mathrm{V}^2_{\pi}(s) = \mathrm{R}(s, \pi_2(s)) +  \gamma \sum_{s'}\mathrm{T}(s, \pi_2(s), s')  \mathrm{V}^1_{\pi}(s')
$${#eq-finite_val_2}
...
$$
\mathrm{V}^h_{\pi}(s) = \mathrm{R}(s, \pi_h(s)) + \gamma \sum_{s'}\mathrm{T}(s, \pi_h(s), s')  \mathrm{V}^{h - 1}_{\pi}(s')
$${#eq-finite_value}
...
Then we can say that a policy $\pi$ is better than policy $\bar{\pi}$ for horizon $h$ if and only if for all $s \in \mathcal S$, $\mathrm{V}_{\pi}^h(s) \geq \mathrm{V}_{\bar{\pi}}^h(s)$ and there exists at least one $s \in \mathcal S$ such that $\mathrm{V}_{\pi}^h(s) > \mathrm{V}_{\bar{\pi}}^h(s)$.
...
$$
\mathrm{V}_{\pi}(s) = \mathbb{E}[\mathrm{R}_0 + \gamma \mathrm{R}_1 + \gamma^2 \mathrm{R}_2 +
...
$$\begin{aligned}
  \mathrm{V}_{\pi}(s) & = \mathbb{E}[\mathrm{R}_0 \mid \pi, S_0 = s] + \gamma  \mathbb{E}[
    \mathrm{R}_1 + \gamma(\mathrm{R}_2 + \gamma \dots))) \mid \pi, S_0 = s]
  \nonumber
  \\
             & = \mathrm{R}(s, \pi(s)) + \gamma\sum_{s'}\mathrm{T}(s, \pi(s), s')\mathrm{V}_{\pi}(s') \; .
\end{aligned}
$${#eq-inf_horiz_value}
...
One way to find an optimal policy is to compute an optimal *action-value function*, $\mathrm{Q}$. For the finite-horizon case, we define $\mathrm{Q}^h(s, a)$ to be the expected value of
...
Similar to our definition of $\mathrm{V}^h$ for evaluating a policy, we define the $\mathrm{Q}^h$ function recursively according to the horizon. The only difference is that, on each step with horizon $h$, rather than selecting an action specified by a given policy, we select the value of $a$ that will maximize the expected $\mathrm{Q}^h$ value of the next state. 

$$\begin{aligned}
  \mathrm{Q}^0(s, a) & = 0                                                                 \\
  \mathrm{Q}^1(s, a) & = \mathrm{R}(s, a) + 0                                                       \\
  \mathrm{Q}^2(s, a) & = \mathrm{R}(s, a) + \gamma \sum_{s'}\mathrm{T}(s, a, s') \max_{a'} \mathrm{Q}^1(s', a')       \\
  \vdots
  \nonumber                                                                       \\
  \mathrm{Q}^h(s, a) & = \mathrm{R}(s, a) + \gamma \sum_{s'}\mathrm{T}(s, a, s') \max_{a'} \mathrm{Q}^{h - 1}(s', a')
\end{aligned}
$$ 
where $(s',a')$ denotes the next time-step state/action pair. We can solve for the values of $\mathrm{Q}^h$ with a simple recursive algorithm called *finite-horizon value iteration* that just computes $\mathrm{Q}^h$ starting from horizon 0 and working backward to the desired horizon $H$. Given $\mathrm{Q}^h$, an optimal $\pi_h^*$ can be found as follows: $$\pi_h^*(s) = \arg\max_{a}\mathrm{Q}^h(s, a) \;\;.$$ which gives the *immediate* best action(s) to take when there are $h$ steps left; then $\pi_{h-1}^*(s)$ gives the best action(s) when there are $(h-1)$ steps left, and so on. In the case where there are multiple best actions, we typically can break ties randomly.
...
Let's consider what would happen if we tried to compute $\mathrm{Q}^4(s, a)$ for all $(s, a)$ by directly using the definition:

-   To compute $\mathrm{Q}^4(s_i, a_j)$ for any one $(s_i, a_j)$, we would need to compute $\mathrm{Q}^3(s, a)$ for all $(s, a)$ pairs.

-   To compute $\mathrm{Q}^3(s_i, a_j)$ for any one $(s_i, a_j)$, we'd need to compute $\mathrm{Q}^2(s, a)$ for all $(s, a)$ pairs.

-   To compute $\mathrm{Q}^2(s_i, a_j)$ for any one $(s_i, a_j)$, we'd need to compute $\mathrm{Q}^1(s, a)$ for all $(s, a)$ pairs.

-   Luckily, those are just our $\mathrm{R}(s, a)$ values.

So, if we have $n$ states and $m$ actions, this is $O((mn)^3)$ work --- that seems like way too much, especially as the horizon increases! But observe that we really only have $mnh$ values that need to be computed: $\mathrm{Q}^h(s, a)$ for all $h, s, a$. If we start with $h=1$, compute and store those values, then using and reusing the $\mathrm{Q}^{h-1}(s, a)$ values to compute the $\mathrm{Q}^h(s, a)$ values, we can do all this computation in time $O(mnh)$, which is much better!
:::
...
An important theorem about MDPs is: in the infinite-horizon case, there exists a stationary optimal policy $\pi^*$ (there may be more than one) such that for all $s \in \mathcal S$ and all other policies $\pi$, we have $$\mathrm{V}_{\pi^*}(s) \ge \mathrm{V}_{\pi}(s) \;\;.$$
...
\WHILE{not converged}
    \FOR{each $s \in \mathcal{S}$ and $a \in \mathcal{A}$}
        \STATE $\mathrm{Q}_{\text{new}}(s,a) \gets R(s,a) + \gamma \displaystyle\sum_{s'} \mathrm{T}(s,a,s')\, \max_{a'} \mathrm{Q}_{\text{old}}(s',a')$
    \ENDFOR
...
-   After executing infinite-horizon value iteration with convergence hyper-parameter $\epsilon$, $$\lVert  \mathrm{V}_{\pi_{\mathrm{Q}_{\text{new}}}} - \mathrm{V}_{\pi^*} \rVert_{\text{max}} < \epsilon \;\; .$$
...
-   As the algorithm executes, $\lVert \mathrm{V}_{\pi_{\mathrm{Q}_{\text{new}}}} - \mathrm{V}_{\pi^*} \Vert_{\text{max}}$ decreases monotonically on each iteration.
