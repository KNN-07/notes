...
\WHILE{not converged}
    \FOR{each $s \in \mathcal{S}$ and $a \in \mathcal{A}$}
        \STATE $Q_{\text{new}}(s,a) \gets R(s,a) + \gamma \displaystyle\sum_{s'} T(s,a,s')\, \max_{a'} Q_{\text{old}}(s',a')$
    \ENDFOR
    \IF{$\displaystyle\max_{s,a}\left|Q_{\text{old}}(s,a)-Q_{\text{new}}(s,a)\right| < \epsilon$}
        \STATE \textbf{return} $Q_{\text{new}}$
    \ENDIF
