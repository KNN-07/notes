...
:::{.column-margin}
We actually can also define the action value functions for a fixed policy $\pi$, denoted as $\mathrm{Q}^{\pi}_h(s,a),$ which represents the expected sum of discounted rewards obtained when taking an action $a$ in state $s$ and following the optimal policy thereafter for the remaining $h-1$ horizon. Similar to
Similar to $\mathrm{V}^{\pi}_{h}(s),$ $\mathrm{Q}^{\pi}_{h}(s,a)$ satisfy the Bellman resursion or Bellman equations introduced earlier. In fact, the two value functions are closely related: for a deterministic policy $\pi$:
$$
\mathrm{Q}^{\pi}_{h}(s,\pi(s)) = \mathrm{V}^{\pi}_{h}(s).
$$

Since we typically care more about action-value functions in order to find the *optimal* policy, we won't be focusing on $\mathrm{Q}^{\pi}(s,a),$ and instead, we will focus more on the optimal action vlaues $\mathrm{Q}^{*}(s,a),$
:::
