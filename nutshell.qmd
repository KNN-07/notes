...
- $ \mathcal{Y} = {+1, 0} $
- $ \hclass = {\sigma(\theta^T x + \theta_0)} $ for values of parameters $ \theta \in \mathbb{R}^d $ and $ \theta_0 \in \mathbb{R} $.
- $ \mathcal{L}(g, y) = \mathcal{L}_{01}(g, h) $
- Proxy loss $ \mathcal{L}_\text{nll}(g, y) = -\Bigl(y \log(g) + (1 - y)\log(1 - g)\Bigr) $

Our learning algorithm has hyperparameter $ \lambda $ and can be written as:

$$
\mathcal{A}(\cal D; \lambda) = \Theta^*(\lambda, \cal D) = \arg\min_{\theta, \theta_0} \frac{1}{|\cal D|} \sum_{(x, y) \in \cal D} \mathcal{L}_\text{nll}\Bigl(\sigma(\theta^T x + \theta_0), y\Bigr) + \lambda |\theta|^2;.
$$

For a particular training data set and parameter $ \lambda $, it finds the best hypothesis on this data, specified with parameters $ \Theta = (\theta, \theta_0) $, written $ \Theta^*(\lambda, \cal D) $ according to the proxy loss $ \mathcal{L}_\text{nll} $.

Picking the best value of the hyperparameter is choosing among learning algorithms based on their actual predictions. We could, most simply, optimize using a single training / validation split, so that $ \cal D = \cal D^\text{train} \cup \cal D^\text{val} $, and we use the real 01 loss:

$$
\begin{aligned}
\lambda^* &= \arg\min_{\lambda} \mathcal{V}(\mathcal{A}_\lambda, \mathcal{L}_{01}, \cal D^\text{val}) \
&= \arg\min_{\lambda} \mathcal{E}(\Theta^(\lambda, \cal D^\text{train}), \mathcal{L}_{01}, \cal D^\text{val}) \
&= \arg\min_{\lambda} \frac{1}{|\cal D_\text{val}|} \sum_{(x, y) \in \cal D_\text{val}} \mathcal{L}_{01}\Bigl(\sigma\Bigl(\theta^(\lambda, \cal D^\text{train})^T x + \theta^*_0(\lambda, \cal D^\text{train})\Bigr), y\Bigr)
\end{aligned}
$$
...
Finally! To make a predictor to ship out into the world, we would use all the data we have, $ \cal D $, to train, using the best hyperparameters we know, and return
