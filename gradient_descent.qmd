...
::: codebox
$\Theta^{(0)} \gets \Theta_{\it init}$ $t \gets 0$ $t \gets t+1$ $\Theta^{(t)} = \Theta^{(t-1)} - \eta \, f'(\Theta^{(t-1)})$ $|f'(\Theta^{(t)})| < \epsilon$ $\Theta^{(t)}$
:::
...
Consider all of the potential stopping criteria for $\proc{1D-Gradient-Descent}$, both in the algorithm as it appears and listed separately later. Can you think of ways that any two of the criteria relate to each other?
:::
...
$\theta^{(0)} \gets \theta_{\it init}$ $\theta_0^{(0)} \gets \theta_{0 {\it init}}$ $t \gets 0$ $t \gets t+1$ $\theta^{(t)} = \theta^{(t-1)} - \eta\left(\frac{1}{n}\sum_{i=1}^n
    ...
    \left({\theta^{(t-1)}}^T x^{(i)} + \theta_0^{(t-1)} -
      y^{(i)} \right)
    \right)$ $\left| J_{\text{ridge}}(\theta^{(t)},\theta_0^{(t)}) - J_{\text{ridge}}(\theta^{(t-1)},
...
::: codebox
$\Theta^{(0)} \gets \Theta_{\it init}$ $t \gets 1$ $T$ randomly select $i \in \{1, 2, \dots, n\}$ $\Theta^{(t)} = \Theta^{(t-1)} - \eta(t) \, \nabla_\Theta f_i(\Theta^{(t-1)})$ $\Theta^{(t)}$
:::
