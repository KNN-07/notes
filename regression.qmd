...
Note that despite stable content, this chapter's PDF notes has two simultaneous conventions for a data matrix -- both the "each row as data" and "each column as data" convention. We are working to unifying these to just "each row as data point"; in other words, $X$ below and in assignments corresponds to the $\tilde{X}$ in the PDF notes.

If there is an inconsistency or confusion, please raise an issue so we can address and correct. Thanks! 
:::
...
In fact, this augementation "fake 1" trick described above is also handy for motivating another way of avoid dealing with $\theta_0$, directly via *centering*. By centering, we mean subtracting the mean of each feature from all the data points; and we do the same for the labels. For example, below is an example of a data set before and after centering:

https://slides.com/shensquared/introml-sp25-lec2#/4/5

<!-- TODO: add data table from lecture here. Perferably into markdown syntax. -->

Recall, ultimately we are interested in finding an optimal fitting hyperplane, parameterized by $\theta$ and $\theta_0$. In other words, we'd like to find $\theta_{\text{aug}$


We claim that, with centererd data, the optimal $\theta_0$ natrually will just be 0, and then find the $\theta$ values that minimize the mean squared error. To see this, a few primers:

1. If the data is centered, meaning that each column of $X$ sums up to one, we must have $X^T\mathbb{1} = 0$.
2. Similarly, for centered data set, mean of the $y$ values is 0, so $Y^T\mathbb{1} = \mathbb{1} ^T Y = 0$. 

Now, plugging in the OLS solution formula, we get:

$$
\begin{aligned} \theta^*=\left[\begin{array}{l}\theta^* \\ \theta_0^*\end{array}\right] & =\left(\left[\begin{array}{l}X^{\top} \\ \mathbb{1}^{\top}\end{array}\right]\left[\begin{array}{ll}X & \mathbb{1}\end{array}\right)^{-\mathbb{1}}\left[\begin{array}{l}X^{\top} \\ \mathbb{1}^{\top}\end{array}\right] Y\right. \\ & =\left(\left[\begin{array}{cc}X^{\top} X & X^{\top} \mathbb{1} \\ \mathbb{1}^{\top} X & \mathbb{1}^{\top} \mathbb{1}\end{array}\right]^{-\mathbb{1}}\left[\begin{array}{l}X^{\top} \\ \mathbb{1}^{\top}\end{array}\right] Y\right)\end{aligned}\
$$

where the last equality follows from the fact that $X^T\mathbb{1} = 0$.





This has the effect of setting $\theta_0$ to 0, and we can then use the same formula as in @eq-ols_solution to find the $\theta$ values that minimize the mean squared error.
