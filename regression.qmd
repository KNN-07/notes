...
Note that despite stable content, this chapter's PDF notes has two simultaneous conventions for a data matrix -- both the "each row as data" and "each column as data" convention. We are working to unifying these to just "each row as data point"; in other words, $X$ below corresponds to the $\tilde{X}$ in the PDF notes.
