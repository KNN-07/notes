...
$$
\theta_{\text{ridge}} = \left({X}^T{X}+ n\lambda I\right)^{-1}{X}^T\tilde{Y}
...
f we do not treat $\theta_0$ specially and instead consider the augmented data matrix $X_\text{aug}$ and parameter vector $\theta_\text{aug}$, then we obtain:

$$J_\text{ridge} (\theta_\text{aug}) = \frac{1}{n} (X_\text{aug} \theta_\text{aug} - Y)^T (X_\text{aug} \theta_\text{aug} - Y) + \lambda \theta_\text{aug}^T \theta_\text{aug}.$$

Taking the gradient of $J_\text{ridge}$ with respect to $\theta_\text{aug}$ yields:

$$\nabla_{\theta_\text{aug}} J_\text{ridge} = \frac{2}{n} (X_\text{aug}^T X_\text{aug} \theta - X_\text{aug}^T Y) + 2 \lambda \theta_\text{aug}. $$

Setting this equal to zero and solving for $\theta_\text{aug}$ yields:

$$\theta_\text{ridge} = (X_\text{aug}^T X_\text{aug} + n \lambda I )^{-1} X_\text{aug}^T Y, $$
