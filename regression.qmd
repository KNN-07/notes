:::{.callout-warning}
We had legacy [PDF notes](https://introml.mit.edu/_static/spring25/notes.pdf) that had mixed conventions for data matrices: "each row as a data point" and "each column as a data point." 

We are standardizing to "each row as a data point." Thus, $X$ in assignments aligns with $\tilde{X}$ in the PDF notes if you've read those.

If you spot inconsistencies, please raise an issue. Thanks!
:::
...
The idea is that, with centered dataset, even if we were to search for an offset term $\theta_0$ to minimize the mean-square-error, it would always fall out to be 0. Intuitively, this makes sense -- if a dataset is centered around the origin, it seems natural that the best fitting hyperplane would go through the origin.
