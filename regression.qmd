...
Our ultimate goal is to find an optimal fitting hyperplane, parameterized by $\theta$ and $\theta_0$. In other words, we aim to find $\theta_{\text{aug}},$ which at this point, involves simply plugging $\theta_{\text{aug}}$ into @eq-ols_solution. 

With centererd dataset, we claim that the optimal $\theta_0$ natrually will fall out to be 0.
